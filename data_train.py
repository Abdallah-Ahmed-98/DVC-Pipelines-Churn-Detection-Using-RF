...
def train_model(X_train, y_train, plot_name, n_estimators, max_depth, class_weight=None):
    ...
    plt.savefig(f'conf-{plot_name}.png', bbox_inches='tight', dpi=300)
    plt.close()  
    ...
    plt.savefig(f'ROC-{plot_name}.png')  # Save ROC curve plot to a file
...
def main():
    ...
    confusion_matrix_paths = [f'./conf-without-imbalance.png', f'./conf-with-class-weights.png', f'./conf-with-SMOTE.png']
    ...
    ## Combine all ROC matrix in one
    ROC_matrix_paths = [f'./ROC-without-imbalance.png', f'./ROC-with-class-weights.png', f'./ROC-with-SMOTE.png']

    ## Load and plot each confusion matrix
    plt.figure(figsize=(15, 5))  # Adjust figure size as needed
    for i, path in enumerate(ROC_matrix_paths, 1):
        im = Image.open(path)
        plt.subplot(1, len(ROC_matrix_paths), i)
        plt.imshow(im)
        plt.axis('off')  # Disable axis for cleaner visualization
